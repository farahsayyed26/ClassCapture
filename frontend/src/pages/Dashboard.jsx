...
const Dashboard = () => {
  ...
  const handleUpload = async (uploadedFile) => {
    ...
    try {
      const response = await axios.post(`https://class-capture-backend.onrender.com/upload`, formData);
  ...
  const renderContent = () => {
    ...
    // REMOVED: if (loading) return <LoadingScreen />; 
    // Logic moved inside the label below
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-12 w-full">
            ...
            <div className="flex flex-col items-center gap-6 w-full">
              <label
                ...
                className={`relative w-full max-xl h-72 border-4 border-dashed rounded-3xl flex flex-col items-center justify-center cursor-pointer transition-all duration-300 ${isDragging ? 'border-brand-main bg-brand-main/5 scale-105' : 'border-slate-200 bg-white hover:border-brand-light'}`}
              >
                <input type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleUpload(e.target.files[0])} disabled={loading} />

                {/* FIX: CONDITIONAL RENDERING INSIDE THE LABEL */}
                {loading ? (
                  <LoadingScreen />
                ) : (
                  <>
    ...
    <div className="flex min-h-screen bg-[#F8FAFC]">
      ...
      <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'md:ml-64' : 'md:ml-20'}`}>
        <header className="h-20 bg-white/80 backdrop-blur-md border-b flex items-center justify-between px-8 sticky top-0 z-20">
          <Menu className="cursor-pointer text-slate-600 hover:text-brand-main" onClick={() => setIsSidebarOpen(!isSidebarOpen)} />
          <div className="flex items-center gap-4 cursor-pointer group" onClick={() => setActiveTab('profile')}>
            <div className="text-right hidden sm:block">
              <p className="text-sm font-bold text-slate-900 pt-[0.8rem]">{user?.displayName || "User"}</p>
            </div>
        ...
        <div className="p-6 md:p-12 min-h-[calc(100vh-80px)]">{renderContent()}</div>
      </main>
    </div>
  );
};
